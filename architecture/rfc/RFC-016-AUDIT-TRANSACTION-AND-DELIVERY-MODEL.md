...
1. PS-009 Event Bus owns asynchronous delivery; Audit owns normalization and canonical persistence.
2. Audit repositories receive the opaque shared `TransactionContext` and resolve Prisma through `PrismaTransactionContextStore`.
3. Same semantic duplicate returns the existing record; a mismatch fails with a typed integrity conflict.
4. PS-009 retry/dead-letter policy applies to Event Bus delivery; production alert thresholds remain operational configuration.
