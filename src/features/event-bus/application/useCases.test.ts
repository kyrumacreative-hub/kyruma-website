...
import type { AuditRecorder, ClaimedDelivery, ClaimedEvent, EventBusRepository, RegisteredHandler } from "../ports/EventBusRepository";
...
class MemoryRepository implements EventBusRepository {
  appended: EventEnvelope[] = []; pending: ClaimedEvent[] = []; deliveries: ClaimedDelivery[] = []; processed: string[] = []; failures: { id: string; retry: Date | null }[] = []; requeued: string[] = []; loseLease = false;
  async append(value: EventEnvelope): Promise<void> { this.appended.push(value); }
  async claimPendingEvents(): Promise<readonly ClaimedEvent[]> { return this.pending; }
  async materializeDeliveries(): Promise<void> {}
  async claimDeliveries(): Promise<readonly ClaimedDelivery[]> { return this.deliveries; }
  async markProcessed(id: string): Promise<void> { if (this.loseLease) throw new LeaseOwnershipLostError("delivery"); this.processed.push(id); }
  async markFailed(id: string, _leaseToken: string, input: { nextRetryAt: Date | null }): Promise<void> { if (this.loseLease) throw new LeaseOwnershipLostError("delivery"); this.failures.push({ id, retry: input.nextRetryAt }); }
...
const record = (overrides: Partial<EventProcessingRecord> = {}): ClaimedDelivery => ({ id: "delivery-1", eventId: "evt-1", consumer: "audit", handler: "record", status: "processing", attemptCount: 1, reprocessCount: 0, firstAttemptAt: now, lastAttemptAt: now, processedAt: null, nextRetryAt: null, errorCode: null, envelope: baseEnvelope, leaseToken: "lease-1", ...overrides });
...
test("materializes handlers independently through the transport boundary", async () => { const repository = new MemoryRepository(); repository.pending = [{ envelope: baseEnvelope, leaseToken: "lease-1" }]; const handlers = new EventHandlerRegistry(); handlers.register({ consumer: "audit", handler: "record", eventType: baseEnvelope.eventType, eventVersion: 1, implementation: { handle: async () => undefined } }); const seen: RegisteredHandler[][] = []; const count = await new DispatchPendingEventsUseCase(repository, { materialize: async (_claim, registrations) => { seen.push([...registrations]); } }, handlers, clock).execute({ workerId: "worker" }); assert.equal(count, 1); assert.equal(seen[0].length, 1); });
test("abandons only the stale dispatcher claim and continues the batch", async () => { const repository = new MemoryRepository(); repository.pending = [{ envelope: baseEnvelope, leaseToken: "stale" }, { envelope: { ...baseEnvelope, eventId: "evt-2" }, leaseToken: "current" }]; const handlers = new EventHandlerRegistry(); const materialized: string[] = []; const count = await new DispatchPendingEventsUseCase(repository, { materialize: async (claim) => { if (claim.leaseToken === "stale") throw new LeaseOwnershipLostError("event"); materialized.push(claim.envelope.eventId); } }, handlers, clock).execute({ workerId: "worker" }); assert.equal(count, 2); assert.deepEqual(materialized, ["evt-2"]); });
...
test("abandons a stale worker without marking its reassigned delivery as failed", async () => { const repository = new MemoryRepository(); repository.deliveries = [record()]; repository.loseLease = true; const handlers = new EventHandlerRegistry(); handlers.register({ consumer: "audit", handler: "record", eventType: baseEnvelope.eventType, eventVersion: 1, implementation: { handle: async () => undefined } }); await new ProcessEventUseCase(repository, handlers, transactions, clock).execute({ workerId: "stale-worker" }); assert.deepEqual(repository.processed, []); assert.deepEqual(repository.failures, []); });
