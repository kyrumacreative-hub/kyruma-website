...
import { AccessInvitationDeliveryError, type AccessInvitationGateway } from "../infrastructure/ClerkAccessInvitationDelivery";
...
export class DeliverPartnerInvitationHandler implements EventHandler {
  ...
  async handle(envelope: EventEnvelope): Promise<void> {
    ...
    const scope = current.invitation.scope;
    if (
      envelope.aggregateType !== "AccessInvitation"
      || envelope.aggregateId !== current.invitation.id
      || envelope.organizationId !== scope.organizationId
      || (envelope.partnerId ?? null) !== (scope.partnerId ?? null)
      || (envelope.workspaceId ?? null) !== (scope.workspaceId ?? null)
      || envelope.correlationId !== current.invitation.correlationId
      || envelope.payload.tokenVersion !== current.invitation.tokenVersion
    ) {
      throw new NonRetryableEventError("INVITATION_EVENT_MISMATCH", "Partner invitation event does not match the persisted invitation.");
    }
    ...
    try {
      ...
      delivered = existing ?? await this.gateway.create({ email: current.invitation.email, invitationId: current.invitation.id, workspaceId: current.invitation.scope.workspaceId, acceptanceUrl: acceptanceUrl.toString(), expiresInDays: Math.max(1, Math.ceil((current.invitation.expiresAt.getTime() - this.now().getTime()) / 86_400_000)) });
    } catch (error) {
      if (error instanceof AccessInvitationDeliveryError && !error.retryable) {
        throw new NonRetryableEventError(error.code, error.message);
      }
      throw error;
    }
