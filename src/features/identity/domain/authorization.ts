...
function scopeMatches(membership: Membership, resource: ResourceScope) {
...
export function authorize(actor: AuthenticatedActor, request: AuthorizationRequest): AuthorizationDecision {
  ...
  const scopedMemberships = activeMemberships.filter((membership) => scopeMatches(membership, request.resource));
