...
export class Partner {
  ...
  constructor(properties: PartnerProperties) { 
    if (!properties.primaryWorkspaceId) throw new MissingPrimaryWorkspaceError(); 
    if (!properties.initialOwnerMembershipId) throw new MissingInitialOwnerError(); 
    ...
    this.currentStatus = PartnerStatus.create(properties.status ?? "pending_approval"); 
  }

  get status() { return this.currentStatus.value; }
  
  approve() { this.transition("approved"); } 
  reject() { this.transition("rejected"); } 
  activateWorkspace() { this.transition("workspace_activated"); } 
  activate() { this.transition("active"); } 
  fail() { this.transition("failed"); }

  private transition(next: PartnerStatusValue) { 
    if (!transitions[this.status].includes(next)) throw new InvalidPartnerStateError(`Cannot transition Partner from ${this.status} to ${next}.`); 
    this.currentStatus = PartnerStatus.create(next); 
  }

  recordEvent(event: PartnerDomainEvent) { this.pendingEvents.push(event); } 
  ...
  // --- Método Añadido ---
  createPrimaryWorkspace(input: {
    workspaceId: string;
    membershipId: string;
    occurredAt: Date;
    correlationId: string;
  }) {
    // Devuelve el objeto base esperado por WorkspaceProvisioner
    return {
