...
import type { PartnerWorkspaceProvisioningPort } from "../../workspace/application/services/WorkspaceProvisioner";
import type { TransactionRunner } from "../../lead-lifecycle/ports/TransactionRunner";

export type PartnerApplicationErrorCode =
  "FORBIDDEN" | "NOT_FOUND" | "CONFLICT" | "PRECONDITION" | "PERSISTENCE";
export class PartnerApplicationError extends Error {
  constructor(
    message: string,
    readonly code: PartnerApplicationErrorCode,
  ) {
    super(message);
  }
}
...
export type PartnerApplicationDependencies = {
  ...
  workspaceProvisioner: PartnerWorkspaceProvisioningPort;
...
function requireAccess(
  context: ResolvedOrganizationContext,
  capability: "partner.create" | "partner.read",
) {
  try {
    requireOrganizationContextAccess(context, capability);
  } catch {
    throw new PartnerApplicationError("Access denied.", "FORBIDDEN");
  }
}
...
function mapInfrastructureError(error: unknown): PartnerApplicationError {
  ...
  return new PartnerApplicationError(
    "Partner Creation infrastructure is unavailable.",
    "PERSISTENCE",
  );
}

function toOutput(partner: {
  id: { value: string };
  code: { value: string };
  leadId: { value: string };
  organizationId: { value: string };
  primaryWorkspaceId: { value: string };
  initialOwnerMembershipId: { value: string };
  status: string;
}): PartnerOutput {
  return {
    id: partner.id.value,
    ...
    workspaceId: partner.primaryWorkspaceId.value,
...
export class CreatePartnerUseCase {
  ...
  async execute(input: CreatePartnerInput): Promise<PartnerOutput> {
    ...
    let result: {
      partner: ReturnType<typeof PartnerFactory.create>;
      created: boolean;
    };

    try {
      result = await this.deps.transactions.run(async (transaction) => {
        const existingByCorrelation = await this.deps.idempotency.find(
          input.correlationId,
          transaction,
        );
        if (existingByCorrelation) {
          const existing = await this.deps.partners.findById(
            existingByCorrelation,
            transaction,
          );
          if (!existing)
            throw new PartnerApplicationError(
              "Partner conversion record is inconsistent.",
              "PERSISTENCE",
            );
          if (existing.organizationId.value !== input.context.organization.id)
            throw new PartnerApplicationError(
              "Partner not found.",
              "NOT_FOUND",
            );
        ...
        const existingByLead = await this.deps.partners.findByLeadId(
          input.leadId,
          transaction,
        );
        if (existingByLead) {
          new PartnerCreationService().validateIdempotency(
            existingByLead.correlationId,
            input.correlationId,
          );
          throw new PartnerApplicationError(
            "A Partner already exists for this Lead.",
            "CONFLICT",
          );
        }

        if (
          !(await this.deps.qualifiedLeads.isQualified(
            {
              leadId: input.leadId,
          ...
          throw new PartnerApplicationError(
            "Lead is not qualified for Partner Creation.",
            "PRECONDITION",
          );
        }
        ...
        partner.recordEvent(
          partnerEvent(
            "PartnerCreationStarted",
            partner.id.value,
            `${input.eventId}:started`,
            input.occurredAt,
            input.correlationId,
          ),
        );
        ...
        partner.recordEvent(
          partnerEvent(
            "InitialMembershipCreated",
            partner.id.value,
            `${input.eventId}:membership`,
            input.occurredAt,
            input.correlationId,
          ),
        );
        ...
        partner.recordEvent(
          partnerEvent(
            "PartnerCreationCompleted",
            partner.id.value,
            `${input.eventId}:completed`,
            input.occurredAt,
            input.correlationId,
          ),
        );
    ...
    if (result.created) {
      try {
        await this.deps.events.dispatch(result.partner.pullDomainEvents());
      } catch (error) {
        throw mapInfrastructureError(error);
      }
    }
...
export class GetPartnerUseCase {
  ...
  async execute(input: {
    ...
    const partner = await this.deps.transactions.run((transaction) =>
      this.deps.partners.findById(input.partnerId, transaction),
    );
    if (
      !partner ||
      partner.organizationId.value !== input.context.organization.id
    )
      throw new PartnerApplicationError("Partner not found.", "NOT_FOUND");
...
export class GetPartnerByLeadUseCase {
  ...
  async execute(input: {
    ...
    requireAccess(input.context, "partner.read");
    const partner = await this.deps.transactions.run((transaction) =>
      this.deps.partners.findByLeadId(input.leadId, transaction),
    );
    if (
      !partner ||
      partner.organizationId.value !== input.context.organization.id
