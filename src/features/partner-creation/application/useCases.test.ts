...
import { CreatePartnerUseCase, GetPartnerByLeadUseCase, GetPartnerUseCase, PartnerApplicationError, type CreatePartnerInput, type PartnerApplicationDependencies } from "./useCases";
...
function dependencies(options: { qualified?: boolean; failSave?: boolean; failDispatch?: boolean } = {}) {
  ...
  const deps: PartnerApplicationDependencies = {
    ...
    workspaceProvisioner: {
      ...
      provision: async (input) => input as unknown as Workspace,
    },
...
const input = (overrides: Partial<CreatePartnerInput> = {}): CreatePartnerInput => ({ context: context(), partnerId: "partner-1", leadId: "lead-1", workspaceId: "workspace-1", workspaceMemberId: "workspace-member-1", membershipId: "owner-1", correlationId: "correlation-1", eventId: "event-1", occurredAt: new Date(), ...overrides });

test("creates an atomic Partner conversion and dispatches after commit", async () => {
  const fake = dependencies(); const result = await new CreatePartnerUseCase(fake.deps).execute(input());
...
test("returns the original conversion for a repeated correlation without dispatching twice", async () => {
  const fake = dependencies(); const useCase = new CreatePartnerUseCase(fake.deps); const first = await useCase.execute(input()); const repeated = await useCase.execute(input());
...
test("requires both authorization and a qualified Lead", async () => {
  await assert.rejects(() => new CreatePartnerUseCase(dependencies({ qualified: false }).deps).execute(input()), (error: unknown) => error instanceof PartnerApplicationError && error.code === "PRECONDITION");
  await assert.rejects(() => new CreatePartnerUseCase(dependencies().deps).execute(input({ context: context(["partner.read"]) })), (error: unknown) => error instanceof PartnerApplicationError && error.code === "FORBIDDEN");
});

test("does not dispatch events when persistence rolls back", async () => {
  const fake = dependencies({ failSave: true }); await assert.rejects(() => new CreatePartnerUseCase(fake.deps).execute(input()), (error: unknown) => error instanceof PartnerApplicationError && error.code === "PERSISTENCE");
...
test("maps post-commit dispatch failures without undoing a committed Partner", async () => {
  const fake = dependencies({ failDispatch: true }); await assert.rejects(() => new CreatePartnerUseCase(fake.deps).execute(input()), (error: unknown) => error instanceof PartnerApplicationError && error.code === "PERSISTENCE");
...
test("reads Partners only within the resolved Organization context", async () => {
  const fake = dependencies(); const created = await new CreatePartnerUseCase(fake.deps).execute(input());
  assert.equal((await new GetPartnerUseCase(fake.deps).execute({ context: context(), partnerId: created.id })).id, created.id);
  assert.equal((await new GetPartnerByLeadUseCase(fake.deps).execute({ context: context(), leadId: "lead-1" })).id, created.id);
  await assert.rejects(() => new GetPartnerUseCase(fake.deps).execute({ context: context(["partner.read"], "org-other"), partnerId: created.id }), (error: unknown) => error instanceof PartnerApplicationError && error.code === "NOT_FOUND");
});
