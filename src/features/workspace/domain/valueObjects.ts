...
export class InvitationExpiry { private constructor(readonly value: Date) {} static create(value: Date, now = new Date()) { if (!(value instanceof Date) || Number.isNaN(value.getTime()) || value <= now) throw new InvalidWorkspaceValueError("Invitation expiry must be in the future."); return new InvitationExpiry(new Date(value)); } isExpired(at: Date) { return at >= this.value; } }
