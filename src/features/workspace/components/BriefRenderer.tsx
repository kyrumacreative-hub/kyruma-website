...
export default function BriefRenderer({ brief }: BriefRendererProps) {
  ...
  async function submitBrief() {
    ...
    try {
      ...
      setError("No se ha podido enviar tu Discovery. Por favor, inténtalo de nuevo.");
    } finally {
      setSending(false);
    }
  }
    ...
    return (
      <div className="flex flex-col justify-center space-y-8 py-16">
        ...
        <div className="space-y-4">
          ...
          <p className="text-xl font-light text-neutral-600 dark:text-neutral-300">Hemos recibido vuestra conversación de Discovery.</p>
        </div>
        <p className="max-w-lg leading-7 text-neutral-500 dark:text-neutral-400">Nuestro equipo revisará lo compartido y os escribirá con una propuesta de siguiente paso.</p>
    ...
    return (
      <section className="space-y-8">
        <div>
          ...
          <h2 className="mt-3 text-3xl font-light text-neutral-900 dark:text-white">Confirma lo que hemos descubierto juntos.</h2>
          <p className="mt-3 text-neutral-600 dark:text-neutral-400">Puedes ajustar cualquier respuesta antes de compartir tu Discovery.</p>
        </div>
        ...
        <div className="flex items-center justify-between gap-4 pt-2">
          ...
          <button type="button" disabled={sending} onClick={submitBrief} className="rounded-full bg-[var(--primary)] px-8 py-3 font-medium text-black transition hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-50">
            {sending ? "Enviando..." : "Compartir Discovery"}
          </button>
        </div>
      </section>
    );
  ...
  return (
    <form onSubmit={(event) => event.preventDefault()} className="grid gap-8 xl:grid-cols-[12rem_1fr]">
      <aside className="hidden border-r border-neutral-200 pr-5 dark:border-neutral-800 xl:block">
        <p className="mb-4 text-xs font-medium uppercase tracking-[0.2em] text-neutral-500">Tu Discovery</p>
